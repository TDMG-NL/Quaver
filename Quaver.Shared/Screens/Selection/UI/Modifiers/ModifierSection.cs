...
namespace Quaver.Shared.Screens.Selection.UI.Modifiers
{
    public class ModifierSection
    {
        ...
        public ModifierSection(int width, Texture2D icon, string name, string subText, Color color, List<SelectableModifier> modifiers)
        {
            ...
            Name = new SpriteTextPlus(FontManager.GetWobbleFont(Fonts.InterSemiBold), name.ToUpper(), 18)
            {
                Parent = Header,
                Tint = color,
            ...
            SubText = new SpriteTextPlus(FontManager.GetWobbleFont(Fonts.InterSemiBold), subText.ToUpper(), 16)
            {
                Parent = Header,
