...
namespace Quaver.Shared.Screens.Results.UI.Tabs.Overview
{
    public class ResultsOverviewTab : ResultsTabContainer
    {
        ...
        /// <summary>
        /// </summary>
        private void CreatePlayedBy() => PlayedBy = new TextKeyValue(ResultsLocalization.Get("Played by"), $"{Processor.Value.PlayerName}", 18, Color.White)
        {
        ...
        private void CreateDateAndTime()
        {
            ...
            DateAndTime = new SpriteTextPlus(FontManager.GetWobbleFont(Fonts.InterSemiBold),
                ResultsLocalization.Get("Played on", Processor.Value.Date.ToShortDateString(), time), 18)
            {
                Parent = ContentContainer,
