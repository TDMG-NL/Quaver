...
namespace Quaver.Shared.Screens.MultiplayerLobby.UI.Games
{
    public sealed class DrawableMultiplayerGame : PoolableSprite<MultiplayerGame>
    {
        ...
        {
            Name = new SpriteTextPlus(FontManager.GetWobbleFont(Fonts.InterSemiBold), "", 22)
            {
                Parent = Panel,
        ...
        {
            PlayerCount = new SpriteTextPlus(FontManager.GetWobbleFont(Fonts.InterSemiBold), "", 20)
            {
                Parent = Panel,
                Alignment = Alignment.BotRight,
                Y = DifficultyRating.Y,
