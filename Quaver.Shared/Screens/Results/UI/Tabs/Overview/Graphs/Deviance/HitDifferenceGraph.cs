    ...
    public class HitDifferenceGraph : Sprite
    {
        ...
        private void CreateAxesValues()
        {
            foreach (var line in Graph.LineData)
            {
                ...
                var text = new SpriteTextPlus(FontManager.GetWobbleFont(Fonts.InterSemiBold), $"{(int) window}", 18,
                    false)
                {
                    Parent = line.Line,
            ...
            var zero = new SpriteTextPlus(FontManager.GetWobbleFont(Fonts.InterSemiBold), $"0", 18,
                false)
            {
